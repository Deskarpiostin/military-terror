# Military Terror source code for Xash3D [![Build Status](https://github.com/Deskarpiostin/hlsdk-portable/actions/workflows/build.yml/badge.svg?branch=master)](https://github.com/Deskarpiostin/hlsdk-portable/actions/workflows/build.yml)

Military Terror is a game about military breaching the big terrorist base.
This repository provides source code of Military Terror.
This repository is based on [hlsdk-portable](https://github.com/FWGS/hlsdk-portable/) by [FWGS](https://github.com/FWGS/).

This repository has/is:

	- **GitHub Actions automatic builds**: Whenever a new commit is uploaded to the repository, the GitHub Actions will automatically start building the game for Android, Linux and MacOS
	- **Used for public**: Used for public releases.
...
```
`mt.dll` and `client.dll` will appear in the `build/dlls/Release` and `build/cl_dll/Release` directories.
...
`mt.dll` and `client.dll` will appear in `dlls/` and `cl_dll/` diretories. The libraries built with msvc6 should be compatible with Windows XP.
