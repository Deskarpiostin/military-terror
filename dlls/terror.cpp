...
const char *CTerror::pGruntSentences[] =
{
...
//=========================================================
// IRelationship - overridden because Alien Grunts and 
// Human Grunt's are Terrorists nemesis.
//=========================================================
int CTerror::IRelationship( CBaseEntity *pTarget )
{
	...
	if( FClassnameIs( pTarget->pev, "monster_human_grunt" ) || ( FClassnameIs( pTarget->pev,  "monster_gargantua" ) ) )
	{
		return R_NM;
	}
...
void CTerror::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_CAUGHT_ENEMY:
		{
			if( FOkToSpeak() )
			{
				SENTENCEG_PlayRndSz( ENT( pev ), "TR_ALERT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
				JustSpoke();
			}
	...
	if( HasConditions( bits_COND_HEAR_SOUND ) )
	{
		...
		if( pSound )
		{
			if( pSound->m_iType & bits_SOUND_DANGER )
			{
				...
				if( FOkToSpeak() )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "TR_GREN", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
					JustSpoke();
				}
	...
	case MONSTERSTATE_COMBAT:
		{
			...
			else if( HasConditions( bits_COND_LIGHT_DAMAGE ) )
			{
				...
				if( iPercent <= 90 && m_hEnemy != 0 )
				{
					...
					if( FOkToSpeak() ) // && RANDOM_LONG( 0, 1 ) )
					{
						//SENTENCEG_PlayRndSz( ENT( pev ), "TR_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
			...
			else if( HasConditions( bits_COND_ENEMY_OCCLUDED ) )
			{
				if( HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) )
				{
					...
					if( FOkToSpeak() )
					{
						SENTENCEG_PlayRndSz( ENT( pev ), "TR_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
						JustSpoke();
					}
					return GetScheduleOfType( SCHED_RANGE_ATTACK2 );
				}
				else if( OccupySlot( bits_SLOTS_HGRUNT_ENGAGE ) )
				{
					...
					if( FOkToSpeak() )// && RANDOM_LONG( 0, 1 ) )
					{
						//SENTENCEG_PlayRndSz( ENT( pev ), "TR_CHARGE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
				...
				else
				{
					...
					if( FOkToSpeak() && RANDOM_LONG( 0, 1 ) )
					{
						SENTENCEG_PlayRndSz( ENT( pev ), "TR_TAUNT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
						JustSpoke();
					}
...
//=========================================================
// CTerrorRepel - when triggered, spawns a monster_terror
...
{
	UTIL_PrecacheOther( "monster_terror" );
...
void CTerrorRepel::RepelUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	*/
	...
	CBaseEntity *pEntity = Create( "monster_terror", pev->origin, pev->angles );
